...
public class UmCarroJa implements Serializable {
    ...
    private GregorianCalendar dataInicioApp;

    private String errorAlugar = "Não Existem Veículos Disponíveis para Alugar.";
    ...
    public UmCarroJa(){
        ...
        this.dataInicioApp = new GregorianCalendar();
    }
    ...
    public GregorianCalendar getDataInicioApp() {
        return dataInicioApp;
    }

    public void setDataInicioApp(GregorianCalendar date){
        this.dataInicioApp = date;
    }
     ...
     */
     public void classificarCliente(Aluguer alug, int classif){
         ...
         double novaClassificacao = (double)(classificAnterior + classif) / (nAlugs);
         Long c = Math.round(novaClassificacao);
         int classifiFinal = c.intValue();
     ...
     * de fim de aluguer.
     */
    public void alugueresEfetuados(){
        for (Map<String, List<Aluguer>> mAlugs : this.alugueres.values()){
            for (List<Aluguer> lAlugs : mAlugs.values()){
                for (Aluguer alug : lAlugs){
                    if (alug.getDataFim().before(this.dataInicioApp) && !alug.getRealizado()){
    ...
    public List<Veiculo> maisProximo(Coordinate destino, ParDatas date, int quantos) throws NaoExistemVeiculosDisponiveisException{
        ...
        if(veiculosOrdenados.isEmpty()){
            throw new NaoExistemVeiculosDisponiveisException(errorAlugar);
        }
   ...
   public List<Aluguer> getAlugueresCliente(String mail) throws NaoEfetuouNenhumAluguerException {
       ...
       if(this.alugueres != null){
           for(Map<String, List<Aluguer>> aux : this.alugueres.values()) {
               if(aux != null) {
                   lista.addAll(checkAlugueres(mail, aux));
               }
           }
       }
       if(lista.isEmpty()){
    ...
    private List<Aluguer> checkAlugueres(String mail, Map<String, List<Aluguer>> aux) {
    ...
    public void classificarClienteJa(String mail, int classificacao){
        try{
            ...
            if (u instanceof Cliente){
                ...
                int classificAnterior = cli.getClassificacao();
                double novaClassificacao = (double)(classificAnterior + classificacao) / nAlugs;
                Long c = Math.round(novaClassificacao);
                int classifiFinal = c.intValue();
                cli.setClassificacao(classifiFinal);
                this.utilizadores.replace(cli.getEmail(), cli.clone());
